...
## 📈 시뮬레이터 백테스트 결과 (2025-12-11 재검증)

### 예측 정확도 (LinearRegression + Ratio 방식)

| 예측 기간 | MAE (%p) | MAPE (%) | RMSE | 신뢰도 등급 |
|-----------|----------|----------|------|-------------|
| 1개월 | 0.17 | 1.05% | 0.18 | HIGH |
| 2개월 | 0.19 | 1.18% | 0.20 | HIGH |
| 3개월 | 0.19 | 1.20% | 0.21 | HIGH |
| 6개월 | 0.28 | 1.70% | 0.32 | GOOD |

### 핵심 결론

1. **LinearRegression이 Ridge보다 45.8% 더 정확**: 백테스트 재검증 결과
2. **Ratio 방식 사용**: GS충전기/시장전체 각각 예측 후 점유율 계산
3. **Bedrock은 인사이트 생성에만 사용**: 예측 정확도에 영향 없음
4. **전체 평균 MAPE 1.14%**: 모든 예측 기간에서 MAPE < 2% 달성

### 시뮬레이터 구조

- **시뮬레이터 1**: 추가 충전기 → 예상 점유율 (LinearRegression 기반)
- **시뮬레이터 2**: 목표 점유율 → 필요 충전기 (LinearRegression 기반 역계산)
- **크로스 검증**: 오차 < 0.01%p (완벽한 일관성)
